...
def create_table():
    dict = {}
    for bird_y in range(0,65,1):
        for dist_pipe_bird in range(-3,69,1):
            for loc_bottom_pipe in range(1,12,1):
                for jumped in range(0,2,1):
                    key = "_"
                    key = key.join([str(bird_y), str(dist_pipe_bird), str(loc_bottom_pipe), str(jumped)])
                    dict[key] = "0,0" #Before "," is state value, after is the # times we've seen it
    return dict

def get_value_and_visits(val,counter):
    val = val.split(",")
    value = float(val[0])
    visits = float(val[1])
    value = value + (counter/(visits+1))
    return value, visits

def update_visits(key,dict,new_vis, place):
    value = dict[key]
    value = value.split(",")
    value[place] = str(new_vis)
    k = value[0] + "," + value[1]
    dict[key] = k
    
def get_next_move(bird_y,bird_pipe_dist,pipe_height,dict,prev_key, score,counter):
    o_bird_y = bird_y
    o_bird_pipe_dist = bird_pipe_dist
    o_pipe_height = pipe_height
    bird_y += 3
    bird_y = bird_y if bird_y < 61 else 60 
    bird_pipe_dist -= 3
    bird_pipe_dist = bird_pipe_dist if bird_pipe_dist > -3 else -3
    key = "_"
    key = key.join([str(bird_y), str(bird_pipe_dist), str(pipe_height), str(0)])
    values = dict[key]
    val1,visits1 = get_value_and_visits(values,counter)
    val1 = val1 + (score * score * 100)
    bird_y -=3 #put it back in current position
    bird_y -=3 #say it is moving up by 3, but may need to change this
    bird_y = bird_y if bird_y > 0 else 0
    key1 = "_"
    key1 = key1.join([str(bird_y), str(bird_pipe_dist), str(pipe_height), str(1)])
    values1 = dict[key1]
    val2, visits2 = get_value_and_visits(values1,counter)
    val2 = val2 + (score * score * 100)
    if val1 < 0 and val2 < 0:
        dict[prev_key] = "-100000.0,1.0"
    k = 0
    if val1 > val2:
        update_visits(key, dict, visits1+1, 1)
        return 0, val1, key
    elif val2 > val1:
        update_visits(key1, dict, visits2+1, 1)
        return 1, val2, key1
    else:
...
def get_state(bird, pipe, dict,prev_key,score,counter):
    y = int(bird.y)
    bird_y = y / 8
    bird_y = bird_y if bird_y > 0 else 0
    bird_y = bird_y if bird_y < 61 else 60  
    bird_pipe_dist =int(pipe.x) - 50
    bird_pipe_dist = bird_pipe_dist / 8
    bird_pipe_dist = bird_pipe_dist if bird_pipe_dist > -3 else -3
    pipe_height = int(pipe.bottom_pieces)
    move, current_val, key = get_next_move(bird_y,bird_pipe_dist,pipe_height,dict,prev_key,score,counter)
    return move, current_val, key

def add_val_path(dict,path):
    for key in path:
        update_visits(key,dict,100000.0,0)
...
def main():
    ...
    pygame.init()
    opener = open('dictionary.json','rb')
    table = json.load(opener)
    opener.close()
    #table = create_table()
    ...
    while True:
        ...
        while not done:
            ...
            if temp.x < 0:
                temp = pipes[1]
            move = current_val = key = 0
            if frame_clock % 15 == 0:
                checking = True
                move, current_val, key = get_state(bird,temp,table,prev_key, score,counter)
                path.append(key)
            if move == 1 and frame_clock % 15 == 0:
            ...
            if pipe_collision or 0 >= bird.y or bird.y >= WIN_HEIGHT - Bird.HEIGHT:
                ...
                reward = (-1000 * (score + 1)) * (score + 1)
                if not checking:
                    move, current_val, key = get_state(bird,temp,table,prev_key,score,counter)
                    sample = reward + (lamb * current_val)
                    total = current_val + lamb *(sample - current_val)
                    update_visits(key, table, total, 0)
                    lamb = lamb * .99
            if frame_clock % 15 == 0:
                sample = reward + (lamb * current_val)
                total = current_val + lamb *(sample - current_val)
            ...
            frame_clock += 1
            counter += .05
        dumper = open('dictionary.json','wb')
        json.dump(table,dumper)
        dumper.close()
